...
# ENTRYPOINT: sanitize NEO4J_URL and start the app
ENTRYPOINT ["sh","-c","raw=${_IMAGE_NEO4J_URL:-${NEO4J_URL:-}}; raw=${raw#\"}; raw=${raw%\"}; raw=${raw#\'}; raw=${raw%\'}; if [ -n \"$raw\" ]; then raw=$(printf '%s' \"$raw\" | sed -E 's#(://)(127\\.0\\.0\\.1|localhost|0\\.0\\.0\\.0)([:/])#\\1host.docker.internal\\3#'); fi; export NEO4J_URL=\"$raw\"; . /opt/conda/etc/profile.d/conda.sh && conda activate rag311 && exec uvicorn app.main:app --host 0.0.0.0 --port 8000"]
